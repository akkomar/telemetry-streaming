...
cat << EOF
{
    "name": "Frecency aggregator - streaming",
    "new_cluster": {
        ...
        "custom_tags": {
            ...
            "Env": "databricks-dev",
            "Type": "streaming",
            "TelemetryJobName": "frecency_aggregator"
        }
    },
    ...
    "spark_jar_task": {
        "main_class_name": "com.mozilla.telemetry.streaming.FrecencyAggregator",
        "parameters": ["--kafkaBroker","${KAFKA_BROKER}", "--outputModelLocation","FIX_ME"]
    }
}
EOF
}

databricks jobs reset --job-id 453 --json "$(create_job_json)"
